...
points2 = detectFASTFeatures(S);
 ...
 figure,imshow(F); hold on; title('  detecting 10 FAST feature points of Sensed image ')
  plot(points1.selectStrongest(10));
 figure,imshow(S); hold on; title('  detecting 10 FAST feature points of Ref image ')
...
[f2, vpts2] = extractFeatures(S, points2);
 figure; imshow(F);title('extract FAST features from sensed image'), hold on
plot(vpts1);
 figure; imshow(S);title('extract FAST features from Ref image'), hold on
 plot(vpts2);
%  figure; imshow(F); hold on;title('  extracting 10 FAST feature points of Ref image ')
%     plot(vpts1.selectStrongest(10),'showOrientation',true);
%  figure; imshow(S); hold on;title('  extracting 10 FAST feature points of Sensed image ')
...
% figure; imshow(F); hold on;title('  Matching 10 FAST feature points of Ref image ')
% plot(matchedPoints1.selectStrongest(10),'showOrientation',true); 
% figure; imshow(S); hold on;title('Matching 10 FAST feature points of Sensed image ')
...
title(ax, 'Matched points from both images Using FAST');
